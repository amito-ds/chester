...
# def color_metric_values(df):
#     color_map = {'green': '32', 'yellow': '33', 'red': '31'}
#
#     for index, row in df.iterrows():
#         color = row['Color']
#         value = row['Metric Value']
#         if color in color_map:
#             df.at[index, 'Metric Value'] = '\033[1;{}m{}\033[0m'.format(color_map[color], value)
#     df.drop(columns=['Color'], inplace=True)
#     return df


# Create a sample dataframe
df = pd.DataFrame({'Metric Name': ['A', 'B', 'C'], 'Metric Value': [1, 2, 3], 'Color': ['green', 'yellow', 'red']})

# df = color_metric_values(df)
...
# #
# analyze_results(organized_results, parameters)
